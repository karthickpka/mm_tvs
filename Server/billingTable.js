...
searchBillingRecord = function(req,res){
    ...
    mongoClient.connect(url, function(err, db) {
            ...
            else if(req.query.startdate && req.query.enddate)
            {
                ...
                //If only Credit is needed
                if(req.query.checked)
                {
                db.collection(billCollectionName).aggregate([{$match: query},{ $project: { item: 1,Name:"$CustName",total: { $subtract:[{$subtract: [ "$MRP", "$Discount" ]},"$Paid"]} } }]).sort({total:-1}).toArray(function(err,result){
                    if (err) console.log(err);//throw err;
                    db.close();
                    res.send(result);})
                }
                else // Provide all billing info
